...
`[Must]` **框架根注入（插件模式路径解析）**：派发 prompt **第一行**必须声明框架绝对根：「框架根：{从 SessionStart 注入的 [插件根] 复制的绝对路径} — 你读到或执行的所有 `pm-workflow/...` 路径均相对此根解析；bash 脚本调用时用该根拼出绝对路径；`outputs/`、`process_record/` 相对项目 cwd。」此规则覆盖 prompt 中所有 `pm-workflow/...` 引用，不逐一枚举。
...
1. **编排器**：派发 prompt 中对分节文件给出具体 fetch 命令 = **「全部 PM 任务 / 全部 Supervisor」通用行 ∪ 本任务行**的章节并集（从下表复制标题）。**阶段4 子角色派发的行选择（杠杆 a）**：「全部 PM 任务」行 ∪ 「阶段4 子角色通用」行 ∪ **该子角色行**（含其在 `prd_expression_standard` / `proto_spec_md` 的对应行）∪ rule_hard「通用 + 六」行（自审/审核场景再加「六.X 对照表」行）；**整改/通用任务未明确子角色时** → 用「阶段4 PM fallback」行；TP 另含 scaffold v2.0 schema 定点 fetch（杠杆 c，Step 1 第 13 项）
   ...
   `[Must]` **doc_query 命令绝对根改写（插件模式，相对该根解析）**：从 SECTION-MAP 表复制 `doc_query.py` 命令写入派发 prompt 时，**必须把命令中两处 `pm-workflow/...` 都改写为绝对路径**——①脚本路径：`python3 {框架根}/pm-workflow/scripts/doc_query.py`；②目标文件参数：`{框架根}/pm-workflow/rules/X.md`（或对应 agents/ 路径）。改写样例（框架根 = `/home/user/.claude/plugins/pm`）：`python3 /home/user/.claude/plugins/pm/pm-workflow/scripts/doc_query.py fetch /home/user/.claude/plugins/pm/pm-workflow/agents/AI产品经理_Agent.md "一、角色身份；二、核心行为准则"`
...
0. `[Must]` **框架根声明（插件模式路径解析）**：prompt **第一行**必须声明框架绝对根：「框架根：{从 SessionStart 注入的 [插件根] 复制的绝对路径} — 你读到或执行的所有 `pm-workflow/...` 路径均相对此根解析；bash 脚本调用时用该根拼出绝对路径；`outputs/`、`process_record/` 相对项目 cwd。」此规则覆盖 prompt 中所有 `pm-workflow/...` 引用，不逐一枚举。
