...
4. **混合任务**：L2 部分走本 skill，L1 部分（如同时改了 `outputs/` 或 `process_record/state.md`）拆出来派 PM Agent — 须分两次 commit（git-copy 模式时 pre-commit hook 会拒绝 L1+L2 混合 commit；插件模式 L2 只读，无需此顾虑）
...
- **`[Should]` CHANGELOG_L2 下游升级日志追加**：本次 L2 改动**影响下游产物 / 行为**（precheck 新增 check / assemble 改动 / gen_scaffold schema / prd_template / 规范硬约束 / scaffold 字段）时，在仓根 `CHANGELOG_L2.md` **顶部追加一条**，含：日期 + commit_short + 类型 + 一句话 + 下游影响 + **「升级后须知」actionable 整改步骤**（跑什么命令 / 预期结果 / 出 WARN 怎么改）。**Why**：插件用户 `/plugin update` 升级后需快速知道"这次升级要我做什么"，逐条读 commit / ssot_anchors 成本高；CHANGELOG_L2 是 commit 的**下游向消费者视图**（curated 整改指引），随 `/plugin update` 下发给插件用户。**触发判定**：改 `precheck_stage*.py` 新增 check / `assemble.py` / `gen_scaffold.py` schema / `prd_template.html` / `rule_hard_constraints.md` 硬约束 / scaffold schema ⇒ 追加；纯 docstring / 注释 / 内部记账 / 文档措辞 ⇒ 可省（标 [FYI] 或不记）。**强度对齐**：每条「升级后须知」标 [Must]（不做破坏产物）/ [Should]（WARN 阶段渐进整改）/ [FYI]（无须动作）。
