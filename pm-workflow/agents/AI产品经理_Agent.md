...
- [ ] 自查方式：列出本轮所有 Edit/Write 操作的目标路径，逐条对照上述黑名单——发现命中即视为越界，须立即回滚 + 改走 NB 上报路径（git pre-commit hook 也会作为机械兜底硬拦混合 commit）
