...
> ⚠️ **存档说明**：本文件是原《Prototype_Design_Spec.md》（原型设计规范）的备份文档，已于规范拆分时保留为备查用途。**不在当前工作流中使用**。当前工作流使用的拆分规范为：`proto_contract.md`、`prd_expression_standard.md`、`proto_spec_md.md`、`proto_platform_*.md`、`proto_cross_platform.md`（`proto_prd_html.md` 已于 2026-04-22 进一步拆分为存档文件）。
...
> 色彩体系来源：**不觉设计系统（白版主题）**。`[Must]` 所有颜色必须通过 CSS 变量引用，prd.html 中禁止直接硬编码 Hex 值。
...
:root {
  /* 文字 */
  --fb-text-primary:   #333333;   /* 标题、主要内容 */
  --fb-text-secondary: #666666;   /* 说明、标签 */
  --fb-text-hint:      #999999;   /* placeholder、次级信息 */
  --fb-text-disabled:  #cccccc;   /* 禁用文字 */

  /* 语义色 */
  --fb-error:          #E60023;   /* 错误/危险语义（唯一用红场景） */
  --fb-selected-light: #FDE6E9;   /* 选中态浅背景 */

  /* 背景 */
  --fb-white:          #ffffff;   /* 卡片、白底 */
  --fb-bg-1:           #fafafa;   /* 最浅层背景 */
  --fb-bg-2:           #f5f5f5;   /* 页面背景、分组背景 */
  --fb-bg-3:           #f1f1f1;
  --fb-bg-4:           #ededed;
  --fb-bg-5:           #e2e2e2;

  /* 线框/分割线 */
  --fb-border-1:       #ebebeb;   /* 轻分割线 */
  --fb-border-2:       #dbdbdb;   /* 标准边框 */
...
| 优先级 | 强度 | 原则 |
|--------|------|------|
| ★★★ | `[Should]` | **主视觉以黑白灰为主**：页面整体色调用 `--fb-text-*` 和 `--fb-bg-*` 构建 |
| ★★★ | `[Must]` | **红色 `--fb-error` 仅用于错误/危险语义**：报错、校验失败、删除/不可逆操作确认 |
| ★★ | `[Recommended]` | **操作主色**：若产品定义未指定产品主色，主按钮使用 `--fb-text-primary`（深色按钮），而非蓝色 |
| ★ | `[Should]` | **彩色图标原则上禁止**：图标默认使用 `--fb-text-secondary` 或 `--fb-text-hint` 灰度色；仅在有明确业务语义（如状态区分、品牌色系要求）时可例外，须在 spec.md 中注明原因 |
...
```
默认态：   ┌────────────────────────┐  高度 44px，圆角 4px（手机）/ 8px（桌面），边框 var(--fb-border-2)
           │ 占位文字（var(--fb-text-hint)） │
           └────────────────────────┘

聚焦态：   ┌════════════════════════┐  边框变 var(--fb-border-2) 加粗 2px，或产品指定主色
           │ 已输入内容（var(--fb-text-primary)） │
           └════════════════════════┘

错误态：   ┌────────────────────────┐  边框变 var(--fb-error)
           │ 已输入内容              │
           └────────────────────────┘
             ⚠ 错误提示文字（var(--fb-error)，12px/19px）

禁用态：   ┌────────────────────────┐  背景 var(--fb-bg-2)，文字 var(--fb-text-hint)
...
```
主按钮（正常）：  [████ 操作文字 ████]   var(--fb-text-primary) 底，白字，高 44px，圆角 4px/8px
主按钮（禁用）：  [░░░░ 操作文字 ░░░░]   var(--fb-text-disabled) 底，var(--fb-text-hint) 字，pointer-events:none
主按钮（加载）：  [⟳ 处理中...      ]   var(--fb-text-primary) 底，白字，按钮内 Spinner，pointer-events:none
次按钮（正常）：  [ 操作文字 ]            白底，var(--fb-border-2) 边框，var(--fb-text-primary) 字
危险按钮（正常）：[ 删除 ]               白底，var(--fb-error) 边框和字
```

> 若产品定义指定了产品主色（如盘八斗 #3A7BFF），主按钮底色和聚焦态边框改用产品主色；在 `:root` 中增加 `--fb-brand` 变量并替换对应位置。
...
```
成功：  ╔══════════════════════╗  左边线 var(--fb-text-secondary)，白底，var(--fb-text-primary) 字
        ║ ✓  操作成功提示文字  ║  持续 2 秒
        ╚══════════════════════╝

警告：  ╔══════════════════════╗  左边线 var(--fb-text-secondary)
        ║ ⚠  警告提示文字      ║  持续 3 秒
        ╚══════════════════════╝
...
| 场景 | 使用方式 |
|------|---------|
| 首屏加载 / 页面主体内容加载 | 骨架屏（`var(--fb-bg-5)` 占位块，圆角 4px，模拟最终布局轮廓，shimmer 动画） |
...
| 主文案 | 14px / Medium / `var(--fb-text-primary)`，说明空态原因（如「暂无笔记」）。**禁止统一用「暂无数据」** |
| 辅助文案 | 12px / Normal / `var(--fb-text-secondary)`，引导下一步操作 |
...
**在 prd.html 中**：每个使用到新增组件的页面帧区块，在帧组右侧放置「组件说明栏」（宽约 300px，`var(--fb-bg-1)` 背景，左边线 3px `#0055CC`），与帧组并排展示。
...
深色模式使用**不觉黑板主题** Token，在 `:root` 中覆盖对应 CSS 变量：

```css
/* 深色模式（黑板主题）— 覆盖 :root 同名变量 */
[data-theme="dark"] {
  --fb-text-primary:   rgba(255, 255, 255, 0.85);  /* 主要文字 */
  --fb-text-secondary: rgba(255, 255, 255, 0.65);  /* 次要文字 */
  --fb-text-hint:      rgba(255, 255, 255, 0.45);  /* 辅助文字 */
  --fb-text-disabled:  rgba(255, 255, 255, 0.25);  /* 禁用文字 */
  --fb-border-1:       rgba(255, 255, 255, 0.15);  /* 轻分割线 */
  --fb-border-2:       rgba(255, 255, 255, 0.15);  /* 标准边框 */
  --fb-white:          #151515;                    /* 卡片背景 */
  --fb-bg-1:           #000000;                    /* 最深背景 */
  --fb-bg-2:           #151515;                    /* 页面背景 */
  --fb-bg-3:           #2a2a2a;                    /* 分组背景 */
  --fb-bg-4:           #454545;                    /* 悬浮层背景 */
  --fb-nav-bg:         rgba(0, 0, 0, 0.3);         /* 导航栏底色 */
}
```

| 白版 Token | 黑板值 | 用途 |
|-----------|--------|------|
| `--fb-bg-1` | `#000000` | 最底层背景 |
| `--fb-bg-2` | `#151515` | 页面背景 |
| `--fb-bg-3` | `#2A2A2A` | 分组/卡片背景 |
| `--fb-bg-4` | `#454545` | 悬浮层/高亮背景 |
| `--fb-white` | `#151515` | 卡片白底替代 |
| `--fb-text-primary` | `rgba(255,255,255,0.85)` | 主要文字 |
| `--fb-text-secondary` | `rgba(255,255,255,0.65)` | 次要文字 |
| `--fb-text-hint` | `rgba(255,255,255,0.45)` | 辅助/占位文字 |
| `--fb-text-disabled` | `rgba(255,255,255,0.25)` | 禁用文字 |
| `--fb-border-1/2` | `rgba(255,255,255,0.15)` | 分割线/边框 |
...
**骨架屏定义**：骨架屏必须是具有真实视觉结构的帧，须包含灰色矩形占位块（`var(--fb-bg-5)`）、分区色块差异或分隔线，能从视觉上传达页面信息层级和布局结构。**纯文字标注卡（仅写"布局同手机帧"等文字说明）不等同于骨架屏**，不可替代视觉帧。
...
> 来源：`APP中间弹框与底部浮框统一交互规范_v2.0`，同时兼容 Android / iOS，无平台差异冲突。
...
> 新增「需要强确认」节点：避免低风险的轻量二选一（如"切换深色模式？"）误入中间弹框；此类场景用 Toggle 开关或 Toast 反馈，无需弹框阻断。
...
- `[Should]` **操作只有 1～2 个明确选择且需要强确认**（确认 / 取消、同意 / 拒绝）
...
- 按钮规则：
  ...
  - `[Should]` 特殊场景允许 3 个按钮，但必须**纵向排列**（如：稍后提醒 / 立即更新 / 忽略此版本）
  - `[Must]` 严禁 4 个及以上按钮
  - `[Must]` 主按钮：确认 / 同意 / 继续（高亮强调）；次按钮：取消 / 放弃（弱化）
  - `[Should]` 单按钮弹框（如"我知道了"）：返回键等同于点击该唯一按钮
- 关闭规则：
...
- 蒙层规则：
  - `[Must]` 选择 / 确认类底部浮框：必须有蒙层，支持点击蒙层关闭
...
- 禁止在加载态时允许用户关闭弹框
- 禁止高风险中间弹框允许蒙层关闭
- 禁止弹框内嵌套跳转到新页面而不关闭弹框（弹框内操作如需跳转页面，必须先关闭当前弹框）
- 禁止弹框堆叠超过 2 层
- 禁止加载状态无超时兜底（无限等待）
- 禁止系统推送类弹框无触发条件地随意弹出
